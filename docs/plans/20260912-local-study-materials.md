...
- ➕ The saved records live in one module-level client store (`src/client/hooks/useLocalStudyMaterial.ts`), not in the editors. A radical that appears under two kanji of one word renders two cards, and per-editor state would let one card keep the old text on screen after the other card saved.
...
- `data-loader.ts`: `export let localStudyMaterials: Record<string, LocalStudyMaterial>` read strictly from the file in the same `Promise.all`, no fallback when the file is missing, same as the other data files. ➕ The file is hand-written, so the read refuses everything the route refuses, with the same shared rules from `src/model/subject-utils.ts`: an unknown field, a wrong value type, an entry with no field. ➕ The rules that need the subject arrays run in `checkLocalStudyMaterialSubjects(records)` at the end of `initRepository`, because the read also runs inside every save: an id that is no WaniKani subject and a `reading_note` on a radical or kana vocabulary. ➕ `findSubjectTypeById(id)` lives here too, next to the arrays it scans, so that check needs no import cycle. New `setLocalStudyMaterials(next)` in the same module, because an exported `let` cannot be assigned from another module. The upsert uses it after a write, tests use it to reset state.
- `src/server/repository/study-material.ts`:
  - `findSubjectTypeById(id): SubjectType | null` scans the four arrays (➕ it sits in `data-loader.ts`). The route calls it first, because it needs the type for the `reading_note` rule, and answers 404 when it is `null`. The upsert trusts the id.
  - `upsertLocalStudyMaterial(subjectId, patch): Promise<LocalStudyMaterial | null>`. Next record = the record read from the file plus the patch: a note field is overwritten, `add_synonym` adds one word, `remove_synonym` drops one word. An empty note string deletes that field. A record with no fields left is deleted from the object. `saveJsonAtomic` writes the whole object with two-space indent and a trailing newline, then `setLocalStudyMaterials`. Returns the saved record, or `null` when the entry was deleted.
...
- `PATCH /api/study-materials`, body `{ id, meaning_note?, reading_note?, add_synonym?, remove_synonym? }`. Every answer has `ok` with `as const`, like `/add-to-anki`, so the client can narrow:
  - ➕ The body never carries the synonym list. `add_synonym` and `remove_synonym` each name one word, and the server applies it to the record it just read from the file. This is a deviation from the first draft, which sent the whole list. The whole list turned every stale client state into lost data: a client that had not seen a change made outside would silently delete it. With an operation that cannot happen, and the client needs no reconciling at all.
  - 400 `{ ok: false, error }`: ➕ the body is not a JSON object (`Invalid body: must be a JSON object`, checked first); id missing or not an integer; no field besides id; unknown key (`meaning_synonyms` is one, it is a record field and no patch field); a value that is not a string; `reading_note` for a radical or kana vocabulary
...
- ➕ `src/client/hooks/useLocalStudyMaterial.ts` is the store of the saved records. It is module level, not React state, because one subject can render on several cards at once.
  ...
  - ➕ `origins` holds the `fromServer` record the subject's session started from. A render with another value means the file changed outside this tab, for example a hand edit or a `git pull` of `data/userdata`. That record always wins, with no exception for a save that is still running: the hook shows `fromServer`, and the next save drops the confirmed entry first. The cost is only on screen. An answer that lands after such a render can stay hidden until the next fetch, and nothing is lost, because the patch is an operation the server merges onto the file.
  ...
  - `patch` is a plain record. There is no list to build, so nothing is built from an old state: `add_synonym` and `remove_synonym` mean the same in any order the saves land.
  - ➕ `saveLocalStudyMaterial` answers `{ done, isLatest }`. `isLatest()` is false once a later save of the same subject and fields started. It lives in the store and not in a component, because two cards can show one note field.
  - ➕ `waitForLocalStudyMaterialSaves()` takes no subject. It waits for every open save and rejects when one failed, because adding one subject to Anki rewrites its components too.
...
- `UserSynonymsRow` props: `subjectId`, `wanikaniSynonyms: string[]`, `localStudyMaterial: LocalStudyMaterial | null`. The local list comes from the store. WaniKani chips are plain. Local chips get a small × button, and a local word WaniKani also has is shown once, without the button. "+ Add Synonym" opens an inline input, Enter adds, Esc closes. Adding sends `add_synonym`, the × sends `remove_synonym`, never a list. On error the list goes back and a toast shows the message. A synonym already in either list is not added twice.
...
- [x] `data/userdata/study_materials_extra.json` stays valid JSON and never holds empty strings, empty lists or empty entries. ➕ fixed: `applyLocalStudyMaterialPatch` now trims the notes and drops blank synonyms, so a list like `["", " yank "]` no longer stores an empty string. New test `blank values never reach the file` checks the whole written file
