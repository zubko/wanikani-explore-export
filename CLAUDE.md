...
- The store keeps one promise chain per subject id, like the server queue. Two saves for one subject would otherwise race: the slower answer would land on top of the newer one. Saves for different subjects still run at the same time. It holds two maps: `confirmed`, the answer of the last finished save, and `shown`, which is `confirmed` with every save that is still in flight applied on top. Every published map is built from the current `shown` object, so an entry another subject saved meanwhile stays.
