...
- The store keeps one promise chain per subject id, like the server queue. Two saves for one subject would otherwise race: the slower answer would land on top of the newer one, and a failed save would roll back a value the other save already stored. Saves for different subjects still run at the same time. The published map is always built from the store value read **after** the answer (`const data = await ...; publish({ ...saved, [id]: data })`). Spreading before the `await` would drop every entry another subject saved in the meantime.
