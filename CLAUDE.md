...
- `confirmed` is a real `Map` and not an object, and `shown` is only ever spread and published in the same tick. In an object literal the spread runs **first** and the `await` after it suspends, so `confirmed = { ...confirmed, [id]: await save() }` wrote back a copy of the other subjects taken before the request started. Saves of two different subjects really do overlap — only saves of the same subject are serialized — so the slower answer erased the faster one, and the next whole-list payload was then built from the stale page record and deleted those synonyms from the file. Never put an `await` inside an object literal that also spreads shared state.
